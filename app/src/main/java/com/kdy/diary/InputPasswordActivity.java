...
public class InputPasswordActivity extends AppCompatActivity {
    ...
    private void inputType(int type){
        switch(type){
            case AppLockConst.ENABLE_PASSLOCK: //잠금설정
                if(oldPwd.isEmpty()){
                    oldPwd = inputedPassword();
                    onClear();
                    etInputInfo.setText("다시 한번 입력");
                }else{
                    if(oldPwd.equals(inputedPassword())){
                        appLock.setPassLock(inputedPassword());
                        setResult(Activity.RESULT_OK);
                        finish();
                    }else{
                        onClear();
                        oldPwd = "";
                        etInputInfo.setText("비밀번호 불일치, 다시 비밀번호 입력");
                    }
                }
                break;
            case AppLockConst.DISABLE_PASSLOCK: //잠금 설정 삭제
                ...
                break;
            case AppLockConst.CHANGE_PASSWORD: //비밀번호 변경
                if(appLock.checkPassLock(inputedPassword()) && !changePwdUnlock){
                    onClear();
                    changePwdUnlock = true;
                    etInputInfo.setText("새로운 비밀번호 입력");
                    ...
                    if(oldPwd.isEmpty()){
                        ...
                        if(oldPwd.equals(inputedPassword())){
                            ...
                            onClear();
                            oldPwd = "";
                            etInputInfo.setText("현재 비밀번호 다시 입력");
                        }
                    }
                }else{
                    etInputInfo.setText("비밀번호가 틀립니다");
